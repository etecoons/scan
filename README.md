...
Secure self-hosted web network scanner and port diagnostics utility.
...
docker run -d \
  ...
  -e SCAN_PIN=your_secret_pin \
  ghcr.io/studio2201/scan:latest
```
...
| :--- | :--- | :--- |
| `PORT` | The network port the web server binds to | `4503` |
| `SCAN_PIN` | Security PIN code required for client authentication | (None) |
| `SCAN_DATA_DIR` | Directory path where persistent data is stored | `/config` |
...
Each container includes a built-in admin tool located in the system path as `scan`. To open the console, execute a shell inside the container:
```bash
docker exec -it <container-name> sh
```
Then, run `scan` to manage the application:
```bash
scan [command]
```
Running `scan` without arguments or running `scan tui` launches the interactive terminal user interface.
