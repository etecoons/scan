# =============================================================================
# scan — Alpine container image (Unraid-compatible)
# =============================================================================
...
#     SCAN_DATA_DIR env var if needed.
#   - Listens on 0.0.0.0:<port> for bridge networking.
#
# Build (Podman):
#   cd ~/projects/studio2201/scan
#   podman build -f Containerfile.ubi -t scan:latest .
#
# Run (Unraid-style):
#   podman run -d --name scan \
#     -p <host-port>:4503 \
#     -v /mnt/user/appdata/scan:/config \
#     -e SCAN_PIN=<your-pin> \
#     scan:latest
#

# Compare size:
#   podman images | grep -E 'scan|studio2201/scan'
# =============================================================================
...
LABEL org.opencontainers.image.title="scan" \
      org.opencontainers.image.description="scan (Alpine, built on Alpine; Unraid-compatible)" \
      org.opencontainers.image.source="https://github.com/studio2201/scan" \
...
COPY --from=builder /src/target/release/sh /usr/local/bin/scan
...
ENV PORT=4503 \
    RUST_LOG=info \
    SCAN_DATA_DIR=/config


EXPOSE 4503

HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD wget -qO- "http://127.0.0.1:${PORT:-4503}/health" >/dev/null || exit 1
